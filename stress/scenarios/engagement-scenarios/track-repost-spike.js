...
export const trackIds = ["3eaab4b9-4412-41bc-8a42-9bbbde9f418c"];
...
export default function () {
  ...
  const trackId = trackIds[__VU % trackIds.length];
  const response = http.post(`${BASE_URL}/tracks/${trackId}/repost`, null, {
    headers: {
  ...
  check(response, {
    ...
    "not own track (no 400)": (r) => r.status !== 400,
    "no 401 unauthorized": (r) => r.status !== 401,
    "not allowed action 403": (r) => r.status !== 403,
